...
| Quotes data     | Supabase Postgres (self-hosted dataset) | Free      |
| Chatbot AI      | Google Gemini 2.0 Flash                 | Free tier |
...
### How it works (localStorage + Supabase)
...
3. If no quote exists, or the stored date is not today → call the
   `get-daily-quote` Edge Function which picks a random quote from
   the Supabase `quotes` table
4. Save the returned quote to `localStorage` with today's date
5. Display the quote

Every user gets their own fresh quote from our own database,
privately stored in their own browser. No external API dependency.
...
- Quote fetched from Supabase `quotes` table via Edge Function
- User can: save to favourites, download as card, share card

### Feature 2 — Search Quotes
- Search by keyword or author name
- Full text search powered by Postgres (fast, accurate, no external API)
- Results from Supabase `quotes` table via Edge Function
...
- `get-daily-quote` Supabase Edge Function picks a random row from `quotes` table
- `useDailyQuote` hook checks localStorage first, calls Edge Function if needed
- Home page displays the quote with save + share buttons
- Basic QuoteCard component built here (reused everywhere)

### Phase 3 — Search
- `search-quotes` Edge Function queries Supabase `quotes` table using Postgres full text search
